---
title: 《智能体设计模式》之并行模式：并发执行与性能提升[译]
date: 2025-10-13
description: 并行模式通过同时执行多个独立任务来突破顺序执行的性能瓶颈。本文翻译自《智能体设计模式》第三章，系统讲解并行模式的核心理念、七大应用场景（信息收集、数据分析、多 API 交互、内容生成、验证核实、多模态处理、A/B 测试），并提供基于 LangChain 和 Google ADK 的完整代码示例。
category: ai
tags: Parallelization, Concurrency, AI Agent, Design Patterns, Agentic AI, LangChain, Google ADK, Multi-Agent, Performance Optimization
cover: https://media.ginonotes.com/images/20251013_agentic_design_patterns_parallelization/chapter03_fig2.jpg
slug: agentic-design-patterns-parallelization-translation
---
...
以下为原书第三章并行模式的内容，原文链接：[Chapter 3: Parallelization](https://docs.google.com/document/u/0/d/1XVMp4RcRkoUJTVbrP2foWZX703CUJpWkrhyFU2cfUOA/mobilebasic?tab=t.0&_immersive_translate_auto_translate=1#heading=h.h2fn0alj7zzv)，译者：[@Gino](https://github.com/ginobefun)

## 并行模式概述

在前面的章节中，我们探讨了用于顺序工作流的提示链以及用于智能决策的路由模式。虽然这些模式很重要，但许多复杂的智能体任务需要 **同时** 执行多个子任务，而非一个接一个地执行。这时 **并行模式** 就变得至关重要。

并行模式涉及同时执行多个组件，例如大语言模型调用、工具使用，甚至整个子智能体（见图
1）。与等待一个步骤完成后再开始下一个步骤不同，并行执行允许独立任务同时运行，这大大缩短了那些可以分解为相互独立部分的任务的总执行时间。
...
![并行模式示例](https://media.ginonotes.com/images/20251013_agentic_design_patterns_parallelization/chapter03_fig1.png)
...
Google ADK 也提供了强大的原生机制来促进和管理智能体的并行执行，显著提升了复杂多智能体系统的效率和可扩展性。ADK
框架的这一内在能力使开发者能够设计并实现让多个智能体并发运行（而非顺序执行）的解决方案。
...
在 LangChain 框架中，通过 LangChain
的表达式语言（LCEL）可以实现并行执行。常见做法是把多个可运行组件组织成字典或列表，并把这个集合作为输入传给链中的下一个组件。LCEL
执行器会并行执行集合中的各个可运行项。
...
要实现此功能，首先需要安装必要的 Python 包（如 langchain、langchain-community
及 langchain-openai 等模型提供库）。同时需要在本地环境中配置所选语言模型的有效
API 密钥，以便进行身份验证。
...
译者注：[Colab 代码](https://colab.research.google.com/drive/1uK1r9p-5sdX0ffMjAi_dbIkaMedb1sTj) 已维护在[此处](https://github.com/ginobefun/agentic-design-patterns-cn/blob/main/codes/Chapter-03-Parallelization-LangChain-Example.py)。
...
译者注：[Colab 代码](https://colab.research.google.com/drive/1gcztL9ebeqGeWl-_0E9FeMcHMOsknB0I) 已维护在[此处](https://github.com/ginobefun/agentic-design-patterns-cn/blob/main/codes/Chapter-03-Parallelization-ADK-Example.py)。
...
![Parallelization Pattern](https://media.ginonotes.com/images/20251013_agentic_design_patterns_parallelization/chapter03_fig2.jpg)
